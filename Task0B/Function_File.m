...
function jacobian_matrices = find_jacobian_matrices(x_1, x_2, x1_dot, x2_dot)
  ...
  jacobian_matrices = {};
  ################## ADD YOUR CODE HERE ######################
    % Compute the Jacobian of x1_dot and x2_dot
    J = jacobian([x1_dot; x2_dot], [x1; x2]);

    % For each equilibrium point, substitute the values of x1 and x2 in the Jacobian
    for i = 1:length(solutions)
        jacobian_matrices{i} = double(subs(J, [x1, x2], solutions(i,:)));
    end
  ############################################################  
endfunction
...
function [eigen_values stability] = check_eigen_values(x_1, x_2, jacobian_matrices)
  ...
  for k = 1:length(jacobian_matrices)
    ...
    eigs = eig(matrix);
        ...
        % Store the eigenvalues in the cell array
    eigen_values{k} = eigs;

        % Check real part of all eigenvalues
    for i = 1:length(eigs)
        if real(eigs(i)) > 0
          flag = 0;  % Set flag to 0 if any eigenvalue has positive real part
          break;
        end
    end
    ############################################################
